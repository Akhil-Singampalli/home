...
<head>
  ...
  <meta name="description" content="Your Name — research, projects, writing, and contact." />
  <!-- Open Graph / Twitter (edit these) -->
  <meta property="og:title" content="Your Name — Portfolio" />
  <meta property="og:description" content="Research, projects, writing, and contact." />
  ...
  <style>
    /* ====== Design tokens ====== */
    :root{
      --bg: #0b0e14;          /* base background (dark) */
      --card:#11151f;         /* card background */
      --text:#e6eaf2;         /* primary text */
      --muted:#9aa3b2;        /* secondary text */
      --acc:#66d9ef;          /* accent */
      --acc-2:#a6e22e;        /* secondary accent */
      --border:#1f2533;       /* subtle borders */
      --maxw: 960px;          /* content width */
      --radius: 20px;         /* rounded corners */
      --shadow: 0 10px 30px rgba(0,0,0,.35);
    }
    @media (prefers-color-scheme: light){
      :root{ --bg:#f7f7fb; --card:#ffffff; --text:#111318; --muted:#4b5563; --border:#e5e7eb; }
    }
    ...
    *,*:before,*:after{ box-sizing:border-box }
    ...
    /* ====== Header / Nav ====== */
    header{ position:sticky; top:0; z-index:10; backdrop-filter:saturate(160%) blur(8px); background: color-mix(in oklab, var(--bg) 80%, transparent); border-bottom:1px solid var(--border) }
    .nav{ display:flex; align-items:center; justify-content:space-between; gap:16px }
    .brand{ display:flex; align-items:center; gap:10px; font-weight:700; letter-spacing:.2px }
    .brand .dot{ width:10px; height:10px; background:var(--acc); border-radius:50% }
    .links{ display:flex; gap:16px; flex-wrap:wrap }
    .links a{ color:var(--muted); font-weight:600 }
    .links a[aria-current="page"], .links a:hover{ color:var(--text) }
    @media (max-width:640px){ .links{ display:none } .menu-btn{ display:inline-flex } }
    .menu-btn{ display:none; border:1px solid var(--border); background:var(--card); color:var(--text); padding:8px 10px; border-radius:10px }
